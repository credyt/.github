...
Credyt is billing infrastructure for AI products: usage-based metering, prepaid wallets, custom credit assets, and a drop-in customer billing portal. Think Stripe for usage-based AI monetization. Credyt authorizes per-request usage _before_ the cost is incurred, so platforms can ship credits, top-ups, subscriptions, and hybrid pricing without building a billing system themselves.
...
- **Control.** Enforce limits. Stop runaway spend. Hard budget ceilings at the customer, agent, and team level. Pre-authorization through the Wallet APIs: check balance _before_ the work starts, block when budgets run out.
...
3. **Run the setup prompt.** Inside your tool, run `npx skills add credyt/ai-skills`, then prompt it to set up Credyt. Credyt's AI skills configure your billing model, wire up the SDK, and stand up your billing portal.
